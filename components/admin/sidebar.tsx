...
export const DesktopSidebar = ({
  ...
  return (
    <>
      <motion.div
        className={cn(
          "h-full px-4 py-4 hidden md:flex md:flex-col bg-gray-900 w-[300px] flex-shrink-0",
          className
        )}
        animate={{
...
interface Links {
    ...
    href: string;
    icon: React.JSX.Element | React.ReactNode;
    onClick?: (event: React.MouseEvent<HTMLAnchorElement>) => void; // Updated to include onClick
  }
  ...
  export const SidebarLink = ({
    ...
    const handleClick = (event: React.MouseEvent<HTMLAnchorElement>) => {
      if (link.onClick) {
        link.onClick(event);  // Call the onClick function from the link
      }
    };
